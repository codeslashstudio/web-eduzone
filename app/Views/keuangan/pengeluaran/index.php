...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
    <div class="bg-gradient-to-br from-red-500 to-orange-500 rounded-2xl p-6 text-white shadow-xl" data-aos="fade-up">
        <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center mb-3">
            <i class="fas fa-arrow-up text-2xl"></i>
        </div>
        <h3 class="text-2xl font-bold mb-1">
            Rp <?= number_format(array_sum(array_column($pengeluaran ?? [], 'jumlah')) / 1000000, 1) ?> Jt
        </h3>
    ...
    <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl p-6 text-white shadow-xl" data-aos="fade-up" data-aos-delay="100">
        ...
        <h3 class="text-2xl font-bold mb-1"><?= count($pengeluaran ?? []) ?></h3>
        <p class="text-white/80 text-sm">Total Transaksi</p>
    </div>

    <div class="bg-gradient-to-br from-orange-500 to-yellow-500 rounded-2xl p-6 text-white shadow-xl" data-aos="fade-up" data-aos-delay="200">
        <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center mb-3">
            <i class="fas fa-hand-holding-usd text-2xl"></i>
        </div>
        <h3 class="text-2xl font-bold mb-1">
            <?php
            $fromBos = array_filter($pengeluaran ?? [], fn($p) => !empty($p['is_from_bos']));
            echo count($fromBos);
            ?>
        </h3>
        <p class="text-white/80 text-sm">Dari Dana BOS</p>
    </div>
</div>
...
<div class="bg-white rounded-2xl shadow-lg overflow-hidden" data-aos="fade-up">
    ...
    <div class="p-6 border-b border-gray-100">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-400 mt-1">Riwayat semua transaksi pengeluaran</p>
            </div>
            <div class="flex items-center gap-3 flex-wrap">
                ...
                <?php if ($canEdit): ?>
                <a href="<?= base_url('keuangan/pengeluaran/add') ?>" class="btn-primary flex items-center space-x-2 px-5 py-2.5 rounded-xl font-semibold text-sm">
        ...
        <table class="w-full">
            <thead>
                <tr style="background: linear-gradient(135deg, var(--color-primary), var(--color-secondary))" class="text-white">
                    ...
                    <th class="px-6 py-4 text-left text-sm font-bold">Tujuan</th>
                    <th class="px-6 py-4 text-left text-sm font-bold">Metode</th>
                    <th class="px-6 py-4 text-left text-sm font-bold">BOS</th>
            ...
            <tbody class="divide-y divide-gray-100" id="tableBody">
                ...
                <tr>
                    <td colspan="<?= $canEdit ? 9 : 8 ?>" class="px-6 py-16 text-center text-gray-400">
                        ...
                        <?php if ($canEdit): ?>
                        <p class="text-sm mt-1">Klik "Tambah" untuk mencatat pengeluaran baru</p>
                        <?php endif ?>
                    </td>
                </tr>
                <?php else: ?>
                ...
                <tr class="hover:bg-gray-50 transition-colors" data-tanggal="<?= date('Y-m', strtotime($p['tanggal_transaksi'])) ?>">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-600"><?= esc($p['tujuan'] ?? '-') ?></td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">
                        <?php if (!empty($p['is_from_bos'])): ?>
                        <span class="px-2 py-1 bg-purple-100 text-purple-700 rounded-lg text-xs font-semibold">BOS</span>
                        <?php else: ?>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center space-x-2">
                            <a href="<?= base_url('keuangan/pengeluaran/edit/' . $p['id']) ?>"
    ...
    <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between">
        <p class="text-sm text-gray-500">
            Total <span class="font-bold text-gray-900"><?= count($pengeluaran ?? []) ?></span> transaksi
        </p>
        <p class="text-sm font-bold text-red-600">
            Total: Rp <?= number_format(array_sum(array_column($pengeluaran ?? [], 'jumlah')), 0, ',', '.') ?>
        </p>
    </div>
</div>
...
<div id="deleteModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
    <div class="bg-white rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl">
        <div class="text-center">
            ...
            <p class="text-gray-600 mb-1">Hapus transaksi pengeluaran:</p>
...
<script>
    ...
    function confirmDelete(id, keterangan) {
        ...
        document.getElementById('deleteForm').action = '<?= base_url('keuangan/pengeluaran/delete/') ?>' + id;

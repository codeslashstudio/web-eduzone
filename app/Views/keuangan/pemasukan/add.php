...
<div class="flex items-center justify-between mb-6">
    <div class="flex items-center space-x-2 text-sm text-gray-500">
        ...
        <a href="<?= base_url('keuangan/pemasukan') ?>" class="hover:underline">Pemasukan</a>
        <i class="fas fa-chevron-right text-xs"></i>
        <span class="font-semibold" style="color:var(--color-primary)">Tambah</span>
    </div>
    <a href="<?= base_url('keuangan/pemasukan') ?>" class="flex items-center space-x-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 font-semibold text-sm">
...
<div class="bg-white rounded-2xl shadow-lg overflow-hidden" data-aos="fade-up">
    <div class="p-6" style="background:linear-gradient(135deg,var(--color-primary),var(--color-secondary))">
        <h2 class="text-xl font-bold text-white flex items-center">
            <i class="fas fa-plus-circle mr-3"></i>Form Tambah Pemasukan
        </h2>
        <p class="text-white/70 mt-1 text-sm">Catat transaksi pemasukan keuangan sekolah</p>
    </div>

    <form action="<?= base_url('keuangan/pemasukan/store') ?>" method="POST" enctype="multipart/form-data" class="p-8">
        <?= csrf_field() ?>

        <!-- Informasi Transaksi -->
        <div class="mb-8">
            <h3 class="text-base font-bold text-gray-900 mb-5 pb-2 border-b flex items-center">
                <span class="w-7 h-7 rounded-lg flex items-center justify-center mr-2 text-white text-xs" style="background:var(--color-primary)"><i class="fas fa-info-circle"></i></span>
                Informasi Transaksi
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="keterangan" rows="3" class="form-input" required placeholder="Contoh: Pembayaran SPP Siswa Kelas X Januari 2026"><?= old('keterangan') ?></textarea>
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Sumber Dana</label>
                    <input type="text" name="sumber" class="form-input" value="<?= old('sumber') ?>" placeholder="Contoh: Siswa Kelas X, Pemerintah">
                </div>
                <div>
                    ...
                    <p class="mt-1 text-sm font-semibold" id="terbilang" style="color:var(--color-primary)"></p>
                </div>
            </div>
        </div>
        ...
        <div class="mb-8">
            <h3 class="text-base font-bold text-gray-900 mb-5 pb-2 border-b flex items-center">
                <span class="w-7 h-7 rounded-lg flex items-center justify-center mr-2 text-white text-xs" style="background:var(--color-secondary)"><i class="fas fa-credit-card"></i></span>
                Detail Pembayaran
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="no_bukti" class="form-input" value="<?= old('no_bukti') ?>" placeholder="Contoh: TF-2026012345">
                </div>
            </div>
        </div>
        ...
        <div class="mb-8">
            <h3 class="text-base font-bold text-gray-900 mb-5 pb-2 border-b flex items-center">
                <span class="w-7 h-7 rounded-lg flex items-center justify-center mr-2 text-white text-xs bg-emerald-500"><i class="fas fa-paperclip"></i></span>
        ...
        <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-100">
            <a href="<?= base_url('keuangan/pemasukan') ?>" class="px-6 py-3 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 font-semibold">
...
<script>
    ...
    document.getElementById('kategori').addEventListener('change',function(){
        ...
        if(label.includes('SPP'))ket.value=`Pembayaran SPP ${bln}`;
        else if(label.includes('BOS'))ket.value=`Penerimaan Dana BOS ${bln}`;
        else if(label.includes('Donasi'))ket.value=`Donasi untuk sekolah`;
    });
